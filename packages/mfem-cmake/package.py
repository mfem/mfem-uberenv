...
class MfemCmake(CMakePackage,CudaPackage):
    ...
    def cmake(self, spec, prefix):
        ...
        cfg.write("#---------------------------------------\n")
        if '+mpi' in spec:
            cfg.write(cmake_cache_entry("MFEM_USE_MPI", "ON"))
            ...
            if os.path.isfile(mpiexe_bin):
                ...
                if self.spec["cmake"].satisfies('@3.10:'):
                    ...
                    cfg.write(cmake_cache_string("MFEM_MPIEXEC", mfem_mpiexec))
                else:
                    cfg.write(cmake_cache_string("MPIEXEC", mfem_mpiexec))
                    cfg.write(cmake_cache_string("MFEM_MPIEXEC", mfem_mpiexec))

                cfg.write(cmake_cache_string("MFEM_MPIEXEC_NP", mfem_mpiexec_np))

            cfg.write(cmake_cache_string("MPICXX", spec['mpi'].mpicxx))

        else:
            cfg.write(cmake_cache_entry("MFEM_USE_MPI", "OFF"))


        cfg.write(cmake_cache_entry("DPREFIX", prefix)),
        cfg.write(cmake_cache_option("DMFEM_USE_MEMALLOC", 'YES'))
        cfg.write(cmake_cache_option("DMFEM_DEBUG", yes_no('+debug')))
        # NOTE: env['CXX'] is the spack c++ compiler wrapper. The real
        # compiler is defined by env['SPACK_CXX'].
        cfg.write(cmake_cache_string("DCXX", env['SPACK_CXX'])),
        cfg.write(cmake_cache_option("DMFEM_USE_LIBUNWIND", yes_no('+libunwind')))
        cfg.write(cmake_cache_option("D%s", (zlib_var, yes_no('+zlib'))))
        cfg.write(cmake_cache_option("DMFEM_USE_METIS", yes_no('+metis')))
        cfg.write(cmake_cache_option("DMFEM_USE_METIS_5", metis5_str,
        cfg.write(cmake_cache_option("DMFEM_THREAD_SAFE", yes_no('+threadsafe')))
        cfg.write(cmake_cache_option("DMFEM_USE_LAPACK", yes_no('+lapack')))
        cfg.write(cmake_cache_option("DMFEM_USE_SUPERLU", yes_no('+superlu-dist')))
        cfg.write(cmake_cache_option("DMFEM_USE_STRUMPACK", yes_no('+strumpack')))
        cfg.write(cmake_cache_option("DMFEM_USE_SUITESPARSE", yes_no('+suite-sparse')))
        cfg.write(cmake_cache_option("DMFEM_USE_SUNDIALS", yes_no('+sundials')))
        cfg.write(cmake_cache_option("DMFEM_USE_PETSC", yes_no('+petsc')))
        cfg.write(cmake_cache_option("DMFEM_USE_PUMI", yes_no('+pumi')))
        cfg.write(cmake_cache_option("DMFEM_USE_GSLIB", yes_no('+gslib')))
        cfg.write(cmake_cache_option("DMFEM_USE_NETCDF", yes_no('+netcdf')))
        cfg.write(cmake_cache_option("DMFEM_USE_MPFR", yes_no('+mpfr')))
        cfg.write(cmake_cache_option("DMFEM_USE_GNUTLS", yes_no('+gnutls')))
        cfg.write(cmake_cache_option("DMFEM_USE_OPENMP", yes_no('+openmp')))
        cfg.write(cmake_cache_option("DMFEM_USE_CONDUIT", yes_no('+conduit')))
        cfg.write(cmake_cache_option("DMFEM_USE_OCCA", yes_no('+occa')))
        cfg.write(cmake_cache_option("DMFEM_USE_RAJA", yes_no('+raja')))
        cfg.write(cmake_cache_option("DMFEM_USE_AMGX", yes_no('+amgx')))
        cfg.write(cmake_cache_option("DMFEM_USE_CEED", yes_no('+libceed')))
        cfg.write(cmake_cache_option("DMFEM_USE_UMPIRE", yes_no('+umpire')))

        if '~static' in spec:
            cfg.write(cmake_cache_option("STATIC", 'NO'))
        if '+shared' in spec:
            cfg.write(cmake_cache_option("SHARED", 'YES'))
        ...
        if '+lapack' in spec:
            ...
            lapack_blas_lib = ld_flags_from_library_list(lapack_blas)
            cfg.write(cmake_cache_string("LAPACK_LIB",lapack_blas_lib))

        if '+superlu-dist' in spec:
            lapack_blas = spec['lapack'].libs + spec['blas'].libs
            lapack_blas_lib = ld_flags_from_library_list(lapack_blas)
            superlu_parmetis_lib = ld_flags_from_dir(
                [spec['superlu-dist'].prefix.lib,
                spec['parmetis'].prefix.lib],
                ['superlu_dist', 'parmetis']),
            cfg.write(cmake_cache_string("SUPERLU_OPT", 'I%s -I%s' %
                (spec['superlu-dist'].prefix.include,
                spec['parmetis'].prefix.include)))
            cfg.write(cmake_cache_string("SUPERLU_LIB", '%s %s' %
                (superlu_parmetis_lib, lapack_blas_lib)))
